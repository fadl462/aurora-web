...
import { getCurrentUser, listProjects } from "@/lib/api";
  ...
  return (
    <aside className="flex flex-col border-r border-border-soft bg-surface p-3.5 md:w-[248px] w-[76px]">
      ...
      <div className="hidden flex-col gap-0.5 md:flex">
        {projects.map((p) => (
          <div key={p.id} className="flex items-center gap-2 rounded-sm px-2.5 py-2 text-[13px] font-medium text-text-muted">
      ...
      <div className="relative mt-auto hidden border-t border-border-soft pt-3.5 md:block" ref={menuRef}>
        ...
        >
          ...
          <div className="min-w-0">
            <div className="truncate text-[13px] font-medium">Fadl&apos;s Workspace</div>
            <div className="truncate text-[11px] text-text-faint">{email ?? "Pro plan"}</div>
          </div>
        </button>
